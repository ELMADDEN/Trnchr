...
- **Live updates**: the page no longer full-reloads every 60s — it polls in place every 15s and swaps just the content, so your scroll position and any half-typed wallet search survive. A newly-arrived trade briefly flashes green in the recent-trades list; the "Live · Xs ago" label in the top-right ticks up between refreshes. Switching timeframe (1H/6H/24H/7D) is still a normal page load, since that's a deliberate action, not a background update.
